...
import { LoginDto } from '../login/dto/login.dto'; // 로그인 시 사용되는 데이터 전송 객체
...
export class UserService {
  ...
  async sendVerificationCode(email: string): Promise<void> {
    ...
    if (response.status !== 200) { // 만약 HTTP 응답 상태가 200이 아니라면
  ...
  async verifyCode(email: string, code: string): Promise<boolean> {
    ...
    if (!response.data.success) { // 인증 실패 시
  ...
  async validateUser(loginDto: LoginDto): Promise<User | null> {
    const { email, password } = loginDto; // 로그인 DTO에서 이메일과 비밀번호를 추출
    console.log('Validating user with email:', email); // 로그: 사용자 검증 시작
    const user = await this.usersRepository.findOne({ where: { email } }); // 이메일로 유저를 데이터베이스에서 검색

    if (user) { // 유저가 존재할 경우
      console.log('User found:', user); // 로그: 유저 정보 출력
      const passwordMatch = await bcrypt.compare(password, user.password); // 입력된 비밀번호와 데이터베이스에 저장된 비밀번호 비교
      console.log('Password match:', passwordMatch); // 로그: 비밀번호 일치 여부 출력
      if (passwordMatch) { // 비밀번호가 일치하면
        return user; // 해당 유저를 반환
      }
    }
    console.log('Invalid user credentials'); // 로그: 유효하지 않은 로그인 정보
    return null; // 유저가 없거나 비밀번호가 일치하지 않을 경우 null 반환
  }
  ...
  async deleteUser(user_id: number): Promise<void> {
    ...
    if (result.affected === 0) { // 삭제된 유저가 없으면
